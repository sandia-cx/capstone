...
// ----------- NOTES --------------------
// Reference point is BDC -- pulse count equals 0 (120) at BDC
...
// ----- Sensor 1 -----
breakoutPin sensor_pin = GPIO_1;     // prox. sensor signal -> GPIO_1    
bool sensor_start;
bool sensor_main;
...
// -----------------------------------------------
// -----------------------------------------------
// 74 pulses between open and close.
// 400 between opens = 68 pulses dr
// -----------------------------------------------
// -----------------------------------------------
...
// exhaust valve close before BDC during cycle 2 -- exhaust valve close after BDC during cycle 1
// const int EVO = 112;
...
void start_function(){
  ...
  attachInterrupt(digitalPinToInterrupt(sensor_pin), pin_ISR_1_start, CHANGE);
  ...
  while ((!emergency_stop) && (!normal_stop) && (!call_main_func)) {
    ...
    if (sensor_start) {
      ...
      if (pulse_count == 114 && (bdc_found) && (min_rpm_reached)){
        emergency_stop = true;
        stop_code_str = 21;
      }
    ...
    rpm = rpm_pulses * 500000 / (currentMillis - startMillis);
    // Serial.print("RPM: ");
    // Serial.println(rpm);
...
void main_function(){
  ...
  attachInterrupt(digitalPinToInterrupt(sensor_pin), pin_ISR_1_main, CHANGE);
...
void pin_ISR_1_start() {
  ...
  // 0.25ms - 4000 rpm ==> 250 micros == 125
  // 1ms - 1000 rpm
  // 2ms - 500 rpm
  ...
  if (interrupt_time - last_interrupt_time > 120) {
    ...
    if ((loop_time > loop_time_prev * 3.5) && (loop_time < loop_time_prev * 7)){
      pulse_count = 0;
...
void pin_ISR_1_main() {
  ...
  // If interrupts come faster than ### micros, assume it's a bounce and ignore
  // 0.25ms - 4000 rpm ==> 250 micros === micros == 125
  // 1ms - 1000 rpm
  // 2ms - 500 rpm
  if (interrupt_time - last_interrupt_time > 120) 
  {
    ...
    if (loop_time > loop_time_prev * 3.5) { 
